...
public class ResearchController {
	...
	public Researcher updateResearcher(@PathVariable("id") final Integer id, @RequestBody Researcher researcher) {
		Optional<Researcher> e = researchRepo.findById(id);
		if(e.isPresent()) {
			Researcher currentResearcher = e.get();
			
			String researcherSurname = researcher.getResearcherSurname();
			if(researcherSurname != null) {
				currentResearcher.setResearcherSurname(researcherSurname);
			}
			
			String researcherName = researcher.getResearcherName();
			if(researcherName != null) {
				currentResearcher.setResearcherName(researcherName);
			}
			
			String researcherEmail = researcher.getResearcherEmail();
